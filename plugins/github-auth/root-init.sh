# shellcheck shell=sh
# Root stage: the only context that holds $GH_PRIVATE_KEY, which entrypoint.sh drops before handing
# control to the agent user.

# github.com, or an Enterprise Server hostname. Exported so gh picks it up in both the refresh loop
# below and the agent's own shell.
export GH_HOST="${GH_HOST:-github.com}"

GH_INITIAL_TOKEN=$("$PLUGIN_DIR/root/mint-gh-token.py")
export GH_INITIAL_TOKEN

# Installation tokens expire after 1h (hard GitHub limit), so re-mint in the background as root and
